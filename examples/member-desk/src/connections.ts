...
const groups = [
  {
    ...
    entries: workspaceGraph.nodes,
  },
  { kind: "conversion", label: "Conversions", entries: workspaceGraph.edges },
  {
    kind: "reference",
    label: "References",
    entries: workspaceGraph.references,
  },
  {
    kind: "operation",
    label: "Operations",
    entries: workspaceGraph.operations,
  },
] as const;
const choices = groups.flatMap(({ kind, entries }) =>
  entries.map(({ name }) => ({ kind, name })),
);
...
export function connectionsPanel() {
  ...
  return `<section class="dependency-explorer" aria-label="Dependency explorer"><div class="panel-heading"><div><span class="eyebrow">REPRESENT IN FIELDWORK</span><h2>What depends on this?</h2></div></div>
    ...
    <label for="dependency-source">Definition to inspect</label><select id="dependency-source">${groups.map(({ kind, label }) => `<optgroup label="${label}">${choices.map((choice, index) => (choice.kind === kind ? `<option value="${index}" ${selected.kind === choice.kind && selected.name === choice.name ? "selected" : ""}>${escapeHtml(choice.name)}</option>` : "")).join("")}</optgroup>`).join("")}</select>
    <div class="dependency-shortcuts" role="group" aria-label="Explore common definitions">${[
      ...
      .join("")}</div>
    <p class="dependency-summary" role="status">${result.dependents.length} ${result.dependents.length === 1 ? "dependent" : "dependents"} · ${direct} direct · ${result.dependents.length - direct} indirect</p>
