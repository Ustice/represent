...
export function structureView(
  ...
  if (structure.kind === "record")
    return `<section class="rx-schema"><h3>Record · ${structure.fields.length} fields</h3><p class="rx-caption">Additional own string keys are rejected.${structure.refined ? " This record also has custom validation that the graph cannot describe." : ""}</p><div class="rx-table-scroll"><table><thead><tr><th>Field</th><th>Representation</th><th>Structure</th><th>Presence</th></tr></thead><tbody>${structure.fields
